...
- **D-027 — Reversed:** Replace direct DeepSeek access after structured presentation calls proved unreliable in the deployed demo.
...
- **D-036 — Accepted:** Use an original five-slide consulting-style deck with action titles, restrained color, direct labels, source notes, speaker notes, and links to the app, repository, and demo-video fallback.
- **D-037 — Accepted:** Configure the deployed app for Ollama Cloud `glm-5.3:cloud` through the existing provider-independent LLM settings because it supports OpenAI-compatible tool calling.
